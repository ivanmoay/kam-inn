    ...
    <section class="section">
        <div class="row">            
            ...
            <div class="col-lg-10">
                <div class="card">
                    <div class="card-body">
                    <h5 class="card-title">Check-In {{$room->room_name}}</h5>

                    <form method="POST" action="/checkin/{{$room->id}}">
                        @csrf
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-10">
                                ...
                                @foreach ($room_pricings as $item)
                                    <option value="{{$item->duration}}|{{$item->price}}">{{$item->duration}} - {{number_format($item->price,2)}}</option>
                                @endforeach                                
                                </select>
                            </div>
                        </div>
                      
                        <x-form-bottom-buttons submit_txt="Check-In" />
                    </form>
